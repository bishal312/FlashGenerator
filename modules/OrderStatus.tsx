      ...
      <div className="w-full max-w-xl bg-[#0d002d] border border-gray-800 rounded-2xl shadow-2xl p-6">
          ...
          {steps.map((step, i) => {
            ...
            return (
              ...
              >
                ...
                {i < steps.length - 1 && (
                  <div className="absolute top-5 left-1/2 translate-x-1/2 w-full h-[2px] -z-10 bg-gray-700"></div>
                )}
              </div>
            );
          })}
        ...
        <div className="bg-[#153739] rounded-lg p-4 space-y-2">
          ...
          <div className="text-sm leading-relaxed text-gray-200 space-y-1">
            <p>
              Flash coin keeper:{" "}
              <span className="text-gray-300">{orderRecord.user.name}</span>
            </p>
            <p>
              Your deposit amount:{" "}
            ...
            <p>
              Flash USDT order amount:{" "}
              <span className="text-gray-300">${orderRecord.toAmount}</span>
            </p>
            <p>
              Your telegram username:{" "}
              <span className="text-gray-300">@{orderRecord.telegramName}</span>
            </p>
            <p>
              Transaction or order id:{" "}
              <span className="text-gray-300">{orderRecord.txId}</span>
            </p>
            <p>
              Date:{" "}
              <span className="text-gray-300">
                {orderRecord.createdAt!.toLocaleDateString()}
              </span>
            </p>
          </div>
        </div>
