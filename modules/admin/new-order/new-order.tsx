    ...
    <div className="min-h-screen bg-gradient-to-b from-[#04171a] to-[#0b1f21] flex items-center justify-center px-4">
      <div className="w-full max-w-md bg-[#0e1f22]/70 border border-gray-800 rounded-2xl shadow-2xl p-8 backdrop-blur-md">
        ...
        <form action={formAction} className="flex flex-col gap-5 w-full">
          ...
          {filteredNetworks.length === 0 ? (
            ...
            filteredNetworks.map((network) => (
              <div
                ...
                className={`w-full cursor-pointer bg-[#1b1539]/70 hover:bg-[#241c4c] rounded-xl border transition-all duration-200 p-4 flex items-center justify-between ${
          ...
          {state.errors?.properties?.token && (
            <FieldError>{state.errors.properties.token[0]}</FieldError>
          )}
