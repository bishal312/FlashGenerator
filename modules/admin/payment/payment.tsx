...
const Payment = ({
  ...
  const [state, formAction, isPending] = useActionState<PaymentFormState, FormData>(
    payment,
    { message: "", success: false, timestamp: Date.now() }
  );
  ...
  return (
    <div className="min-h-screen w-full flex flex-col items-center justify-center bg-gradient-to-b from-[#061b19] to-black px-4">
      <div className="bg-[#111111] border border-gray-800 py-8 px-6 max-w-2xl rounded-2xl flex flex-col items-center gap-6 w-full shadow-lg shadow-black/40">
        {/* Title */}
        <h1 className="text-2xl font-bold text-gray-100 tracking-wide">Payment</h1>
        ...
        <div className="w-full">
          <form action={formAction} className="flex flex-col gap-4">
            ...
            <input type="hidden" name="depositAddress" value={depositAddress} required />
            <input type="hidden" name="depositQrCodeUrl" value={depositQrCodeUrl} required />
        ...
        <p className="text-gray-300 font-medium">
          ...
          <span className="text-gray-100 font-semibold">{depositedAmount} TRC</span>
        </p>
      </div>
    </div>
  );
